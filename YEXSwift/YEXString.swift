...
public extension YEXProtocol where T == String {
    ...
    /// - Returns: 字符串的首字母
    func pinyexnInitials(_ isUpper: Bool = true) -> String {
        let pinyexn = toPinyexn(false).components(separatedBy: " ")
        let initials = pinyexn.compactMap { String(format: "%c", $0.cString(using:.utf8)![0]) }
