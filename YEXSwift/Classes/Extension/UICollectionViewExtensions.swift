...
public extension UICollectionView {
    ...
    func dequeueReusableSupplementaryView<T: UICollectionReusableView>(ofKind kind: String, withClass name: T.Type, for indexPath: IndexPath) -> T {
        guard let cell = self.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: String(describing: name), for: indexPath) as? T else {
    ...
    func register<T: UICollectionReusableView>(supplementaryViewOfKind kind: String, withClass name: T.Type) {
        self.register(T.self, forSupplementaryViewOfKind: kind, withReuseIdentifier: String(describing: name))
    }
    /// 根据类名注册重用池中的cell
    ...
    func register<T: UICollectionViewCell>(cellWithClass name: T.Type) {
        self.register(T.self, forCellWithReuseIdentifier: String(describing: name))
    }
    /// 根据类名获取重用池中的区头视图
    ...
    func safeScrollToItem(at indexPath: IndexPath, at scrollPosition: UICollectionView.ScrollPosition, animated: Bool) {
        guard indexPath.item >= 0 &&
            indexPath.section >= 0 &&
                ...
                indexPath.item < self.numberOfItems(inSection: indexPath.section) else {
                return
        }
