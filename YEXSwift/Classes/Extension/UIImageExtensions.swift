...
public extension UIImage {
    ...
    ///根据颜色生成图片
    convenience init(color: UIColor, size: CGSize,radius: CGFloat) {
        let image = UIImage.init(color: color, size: size).roundCorners(radius)
        guard let aCgImage = image.cgImage else {
            self.init()
            return
        }
            ...
            if let imageData = newImage.jpegData(compressionQuality: 1.0) {
                ...
                while (sizeOriginKB > maxSize && resizeRate > 0.1) {
                    if let data = newImage.jpegData(compressionQuality: CGFloat(resizeRate)) {
                        ...
                        resizeRate -= 0.1
                        print("压缩后图片--大小:\(sizeOriginKB)--size:\(newSize)")
...
// MARK:- gif 加载
public enum printDataType: String {
    case gif    = "gif"
    case png    = "png"
    case jpeg   = "jpeg"
    case tiff   = "tiff"
...
public extension UIImage {
    ...
    static func checkImageprintDataType(data: Data?) -> printDataType {
        guard data != nil else {
            return .defaultType
        }
        let c = data![0]
...
//MARK: --- runtime
public extension UIImage {
    private struct YIRuntimeKey {
        static let saveBlockKey = UnsafeRawPointer.init(bitPattern: "saveBlock".hashValue)
    }
    private var saveBlock: ((Bool)->())? {
        set {
            objc_setAssociatedObject(self, YIRuntimeKey.saveBlockKey!, newValue, .OBJC_ASSOCIATION_COPY_NONATOMIC)
        }
        get {
            return objc_getAssociatedObject(self, YIRuntimeKey.saveBlockKey!) as? (Bool) -> ()
        }
    }
}
